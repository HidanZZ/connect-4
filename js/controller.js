...
var gameWidth;
var gameHeight;
var game = new Phaser.Game(0, 0, Phaser.CANVAS, "game-canvas", {
	preload: preload,
	create: create,
	update: update,
	currentState: "ready",
...
function create() {
	...
	gameHeight =
		ROW_COUNT * (CELL_SIZE + CELL_PADDING) +
		CELL_PADDING +
...
function update() {
	...
	if (!currentPlayerIsHuman()) {
		...
		if (connect4.currentPlayer == 1) {
			...
			workerAI.postMessage([
				AI1Difficulty,
				1,
				JSON.parse(JSON.stringify(connect4.board)),
			]);
		} else {
...
function currentPlayerIsHuman() {
	...
	if (player1Type == PlayerType.HUMAN && connect4.currentPlayer == 1) {
		humanPlayer = true;
	}
	if (player2Type == PlayerType.HUMAN && connect4.currentPlayer == 2) {
		humanPlayer = true;
	}
